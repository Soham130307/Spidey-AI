...
async function pollBackend() {
    ...
    try {
        ...
        const data =
            await response.json();
            const userCommand = data.user_command || "";
const spideyMessage = data.spidey_message || "";

if (userCommand) {
    const userBox = $("userCommand");

    if (userBox) {
        userBox.textContent = '"' + userCommand + '"';
    }
}

if (spideyMessage) {
    const responseBox = $("spideyResponse");

    if (responseBox) {
        responseBox.textContent = spideyMessage;
    }
}
...
async function sendTypedCommand(
    command
) {
    ...
    let wokeSpidey = false;

    for (const wakeWord of wakeWords) {

        if (
            lowerCommand === wakeWord
        ) {
    ...
    try {
        ...
        log(
            "SPIDEY: Command connection failed."
        );
        ...
        button.addEventListener(
            ...
            async () => {
                ...
                try {
                    ...
                    log(
                        "CHUP â€” Python bridge unavailable."
                    );
                }
...
// ============================================================
// ðŸ•·ï¸ SPIDEY EXPERIENCE UPGRADE
// ============================================================
// Adds:
// 1. Startup sequence
